...
if (DATABOX_TESTING) {
    ...
    store.TSBlob.Observe("helloWorldActuator").then((emitter) => {
        listenToActuator(emitter);
    });
} else {

    const hcatobj = JSON.parse(process.env[`DATASOURCE_helloWorldActuator`]);
    console.log("OK HAVE hcatobj", JSON.stringify(hcatobj,null, 4));
    ...
    console.log("HAVE metadata", JSON.stringify(metadata,null,4));
...
app.get('/ui/actuate', (req, res) => {
    ...
    return new Promise((resolve, reject) => {
        store.TS.Write("helloWorldActuator", { msg: `${Date.now()}: databox actuation event` }).then(() => {
